...
module["exports"] = {
    ...
    run: async (client, message) => {
        ...
        if(message.content.replace('!', '').startsWith(`<@${client.user?.id}>`)){
            ...
            .setThumbnail("https://i.imgur.com/g42FP95.png")
            .setDescription(client.FormatEmoji(`## 👋 Olá ${message.author.username}!\n>>> - Meu prefixo nessa dimensão é **\`${client.prefix}\`**\n- Você pode utilizar **\`${client.prefix}ajuda\`** para ver todos os meus comandos.`))
            .setColor('Blue')

            const buttons = MeiButton([
                {label: "Servidor de Suporte", style: "Link", url: "https://discord.gg/WRka7bGqQ2"},
            ])
